...
func RootDomain(domain string) string {
	if strings.Contains(domain, "@") || len(strings.Split(domain, ".")) <= 1 {
		return ""
	}
...
func GetDomain(uri string) string {
	if par, err := ParseUrl(uri); err == nil {
		return par.Hostname
	}
	return ""
}
...
func GetHostName(uri string) string {
	if parse, err := ParseUrl(uri); err == nil {
		return fmt.Sprintf("%s:%s", parse.Hostname, parse.Port)
	}
	return ""
}
...
// IsLink 判断是否为链接
func IsLink(uri string) bool {
	if ps, err := ParseUrl(uri); err == nil {
		return ps.Scheme != "" && ps.Hostname != "" && ps.Port != ""
	}
	return false
}
...
func GetRootLink(uri string) string {
	var rootLink string
	// 格式化URL
	if parse, err1 := ParseUrl(uri); err1 != nil {
		return ""
		//	协议与默认端口匹配的情况
	} else {
		if parse.Scheme == "http" && parse.Port == "80" || parse.Scheme == "https" && parse.Port == "443" {
			rootLink = fmt.Sprintf("%s://%s", parse.Scheme, strings.ToLower(parse.Hostname))
		} else {
			rootLink = fmt.Sprintf("%s://%s:%s", parse.Scheme, strings.ToLower(parse.Hostname), parse.Port)
		}
	}
	return rootLink
}
...
func FormatUrl(uri string) string {
	if parse, err1 := ParseUrl(uri); err1 == nil {
		if parse.Path == "/" && parse.Query == "" {
			return GetRootLink(uri)
		}
	}
	return uri
}
...
func GetAllTagA(link, htmlContent string) []string {
	var hrefs []string
	if link == "" {
		return hrefs
	}
		...
		doc.Find("a").Each(func(i int, s *goquery.Selection) {
			...
			if href, exists := s.Attr("href"); exists {
				if strings.Contains(href, "http") {
					hrefs = append(hrefs, FormatUrl(strings.TrimSpace(href)))
				} else if strings.HasPrefix(href, "//") && GetDomain(fmt.Sprintf("http:%s", href)) != "" {
					if res, err1 := ParseUrl(link); err1 != nil {
						hrefs = append(hrefs, FormatUrl(strings.TrimSpace(strings.TrimSuffix(fmt.Sprintf("%s:%s", res.Scheme, href), "/"))))
					}
				} else if !strings.HasPrefix(href, "javascript") && href != "/" && !strings.HasPrefix(href, "#") {
					hrefs = append(hrefs, FormatUrl(strings.TrimSpace(fmt.Sprintf("%s/%s", link, strings.TrimPrefix(href, "/")))))
				}
			}
		})
	}
...
// GetAllJs 获取所有Js列表
func GetAllJs(rootLink, htmlContent string) []string {
	var scripts []string
	if rootLink == "" {
		return scripts
	}
		...
		doc.Find("script").Each(func(i int, s *goquery.Selection) {
			...
			if src, exists := s.Attr("src"); exists {
				if strings.Contains(src, "http") {
					scripts = append(scripts, strings.TrimSpace(src))
				} else if strings.HasPrefix(src, "//") && GetDomain(fmt.Sprintf("http:%s", src)) != "" {
					if res, err1 := ParseUrl(src); err1 != nil {
						scripts = append(scripts, strings.TrimSpace(fmt.Sprintf("%s:%s", res.Scheme, src)))
					}
				} else if strings.HasPrefix(src, "/") {
					scripts = append(scripts, strings.TrimSpace(fmt.Sprintf("%s/%s", rootLink, strings.TrimPrefix(src, "/"))))
				}
			}
		})
	}
...
func IsJsFile(uri string) bool {
	par, err1 := ParseUrl(uri)
	if err1 != nil {
		return false
	}
	return strings.HasSuffix(par.Path, ".js")
}

// IsCssFile 是否为Css文件
func IsCssFile(uri string) bool {
	par, err1 := ParseUrl(uri)
	if err1 != nil {
		return false
	}
	return strings.HasSuffix(par.Path, ".css")
}

// IsMediaFile 是否为媒体文件
func IsMediaFile(uri string) bool {
	par, err1 := ParseUrl(uri)
	if err1 != nil {
		return false
	}
	return strings.HasSuffix(par.Path, ".ico") || strings.HasSuffix(par.Path, ".jpg") || strings.HasSuffix(par.Path, ".png") || strings.HasSuffix(par.Path, ".mp3") || strings.HasSuffix(par.Path, ".mp4")
}
